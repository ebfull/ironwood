...
/-- **The captured random single-action verifying key is fully derived.** The cross-capture URS
...
theorem vk_eq_derived : vk = derivedActionVk shape capturedURS := by
  ...
  · rw [randomSingle_uses_same_fixedCommitments]; exact hfcf
  · rw [randomSingle_uses_same_permutationCommonCommitments]; exact hpcf
