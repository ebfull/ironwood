...
`dump_vesta_lean_fixture`), and `Fixture.fingerprint_matches` runs the decidable comparison
...
and captured URS, then proves it equal to the captured `VerifyingKey` field-for-field. What
remains at the fixture-generation boundary is provenance of the Rust capture and its selection
of Orchard's canonical Post-NU6.3 `PinnedVerificationKey`, together with Halo2's pinned-key byte
