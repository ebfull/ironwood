...
some fingerprint-match theorem. Only that direction (deployed-accepts ⊆ model-accepts) is
soundness-relevant; completeness is not a goal.

## The boundary at the derived key

The statements of record are the `nonInteractiveFingerprint_matches_derived` theorems
(`Fixtures/SingleAction/Boundary.lean`, `Fixtures/MultiAction/Boundary.lean`): the challenges
...
URS (`Keygen/Certificate.lean`, transported to the multi-action capture in
`Fixtures/MultiAction/VkCertificate.lean`). This ties the σ- and fixed-column polynomial content
...
discrete-log-solving collision. On the honest captures `capturedMsm_eval_eq_zero` additionally
evaluates the g-block to the identity against the dumped points.
...
(`halo2_proofs/unstable-verifier-fingerprint`); `.github/workflows/fixtures.yml` regenerates the
fixtures in CI from the pinned release, making the capture path release-pinned. The captured
fixture itself — the `2048` g-scalars, `100` commitment terms, `22` challenges, and the
`VerifyingKey` — is loaded in `Fixtures/SingleAction/Fixture.lean` (auto-generated by
`dump_vesta_lean_fixture`); `Fixture.fingerprint_matches` runs the decidable comparison
`MsmMatch (assemble vk derivedInstanceCommitment ps ch) capturedMsm` by `native_decide` over
actual Vesta points, and the boundary statements above restate it at the derived key and
schedule. The generator emits every distinct affine coordinate, validates it on `Vesta.curve`,
and supplies the complete Halo2 URS; `capturedMsm_eval_eq_zero` then computes the captured MSM
to the identity and `assembledMsm_eval_eq_zero` transfers that result to Lean's assembly. The
comparison against Orchard's canonical Post-NU6.3 `PinnedVerificationKey` and Halo2's pinned-key
byte serialization/Blake2b derivation remain the fixture-generation boundary rather than being
